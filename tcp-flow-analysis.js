...
import { WebGLPacketRenderer } from './src/rendering/webgl-packet-renderer.tcpflow.js';
import { renderCirclesWebGL } from './src/rendering/circles-webgl-bridge.tcpflow.js';
import { BinColumns, isBinColumns, asArrayLike } from './src/data/bin-columns.tcpflow.js';
...
// WebGL packet renderer (replaces SVG circles in packet view; tcp-flow only).
let webglPacketRenderer = null;
let webglHoverIndex = -1;
let webglPacketMargin = null; // {top,right,bottom,left} captured for hover math
 ...
 *
 * Memory-tuned for the parquet-loaded dataset (8M+ bins): the merged group
 * object only carries fields actually read downstream (no spread of source),
 * and the per-merge sub-collections (ipPairs/allIPs/originalPackets) are
 * lazily allocated only when a group truly merges >1 distinct pair. This
 * keeps the transient peak during the merge loop ~5x smaller than before.
 */
function collapseSubRowsBins(binned, collapsedIPs) {
    ...
    for (const d of merge) {
        ...
        if (!g) {
            // Build the merged group with ONLY the fields downstream consumers
            // read. Materialize derived values (binCenter/binStart/binEnd) so
            // the group doesn't depend on the source row's getters.
            g = {
                src_ip: d.src_ip,
                dst_ip: d.dst_ip,
        ...
        g.count += (d.count || 1);
        if (d.totalBytes) g.totalBytes += d.totalBytes;
        if (Array.isArray(d.originalPackets) && d.originalPackets.length) {
            if (!g.originalPackets) g.originalPackets = [];
            for (let i = 0; i < d.originalPackets.length; i++) g.originalPackets.push(d.originalPackets[i]);
        }
        const pk = makeIpPairKey(d.src_ip, d.dst_ip);
        if (g._firstPair === null && g._seenPairs === null) {
            // First pair seen — record it without allocating Set/array.
            g._firstPair = pk;
            // Defer ipPairs/allIPs alloc until a *second* distinct pair arrives.
        } else if (g._firstPair !== null && g._firstPair !== pk) {
            // Promote to multi-pair mode; allocate the collections now.
            g._seenPairs = new Set();
            g._seenPairs.add(g._firstPair);
            g._seenPairs.add(pk);
            g.ipPairs = [
                { src_ip: g.src_ip, dst_ip: g.dst_ip, count: g.count - (d.count || 1) },
                { src_ip: d.src_ip, dst_ip: d.dst_ip, count: d.count || 1 }
            ];
            g.allIPs = new Set();
            if (g.src_ip) g.allIPs.add(g.src_ip);
            if (g.dst_ip) g.allIPs.add(g.dst_ip);
            if (d.src_ip) g.allIPs.add(d.src_ip);
            if (d.dst_ip) g.allIPs.add(d.dst_ip);
            g._firstPair = null;
        } else if (g._seenPairs !== null && !g._seenPairs.has(pk)) {
            g._seenPairs.add(pk);
    ...
    for (const g of groups.values()) {
        // Single-pair groups never paid for the collections — synthesize a
        // minimal ipPairs so downstream `d.ipPairs || [{...}]` keeps working.
        if (g._firstPair !== null) {
            g.ipPairs = [{ src_ip: g.src_ip, dst_ip: g.dst_ip, count: g.count }];
            g.allIPs = null; // downstream tolerates null/undefined
        }
        g._firstPair = null;
        g._seenPairs = null;
        pass.push(g);
...
// Per-render cache for scroll-driven viewport virtualization. The packet view
// can hold millions of binned rows for thousands of IP rows; building DOM
// circles for every row OOMs the browser. We instead filter `binned` down to
// rows whose IP row falls inside the scroll viewport (plus a 1-viewport buffer
// above and below) and re-run the render on scroll. `binned` is stashed by
// reference, not copied.
const _circlesViewport = {
    layer: null,
    binned: null,
    rScale: null,
    scrollAttached: false,
    rafPending: false,
    lastFirstVisibleIp: null,
    lastLastVisibleIp: null,
};

function _circlesViewportIPSet() {
    const chartContainer = document.getElementById('chart-container');
    const positions = state.layout && state.layout.ipPositions;
    if (!chartContainer || !positions || positions.size === 0) return null;

    const scrollTop = chartContainer.scrollTop || 0;
    const viewportH = chartContainer.clientHeight || 0;
    if (!viewportH) return null;

    // ipPositions store Y in mainGroup-local coordinates; mainGroup is
    // translated by ~margin.top. A 1-viewport buffer above and below absorbs
    // the offset and gives smooth scroll without re-rendering each frame.
    const buffer = viewportH;
    const yMin = scrollTop - buffer;
    const yMax = scrollTop + viewportH + buffer;

    const visible = new Set();
    for (const [ip, y] of positions) {
        if (y >= yMin && y <= yMax) visible.add(ip);
    }
    return visible;
}

function _renderCirclesViewport(transitionOpts) {
    const { layer, binned, rScale } = _circlesViewport;
    if (!layer || !binned) return;

    const useWebGL = !!webglPacketRenderer;
    // Visible-IP filter is required for both SVG and WebGL paths. SVG needs
    // it to bound DOM-node count. WebGL needs it because collapseSubRowsBins
    // run on 8 M+ rows blows the heap (multi-GB transient peak).
    // For BinColumns we use a column-aware filter that never materializes
    // row objects.
    const visibleIPs = _circlesViewportIPSet();
    let filteredBinned;
    if (visibleIPs && isBinColumns(binned)) {
        filteredBinned = binned.filterByIpRowSet(visibleIPs);
    } else if (visibleIPs) {
        filteredBinned = binned.filter(d => d && (visibleIPs.has(d.src_ip) || visibleIPs.has(d.dst_ip)));
    } else {
        filteredBinned = binned;
    }

    const data = collapseSubRowsBins(filteredBinned, state.layout.collapsedIPs);

    if (useWebGL) {
        renderCirclesWebGL(layer, data, {
            xScale,
            rScale,
            flagColors,
            RADIUS_MIN,
            ...
            d3,
            webglRenderer: webglPacketRenderer
        });
        _drawWebGLPackets();
    } else {
...
/**
 * Trigger a WebGL redraw using the current xScale and chart-container scroll.
 * Cheap — uniforms only, no buffer re-upload.
 */
function _drawWebGLPackets() {
    if (!webglPacketRenderer || state.ui.renderMode !== 'packets') return;
    const container = document.getElementById('chart-container');
    if (!container) return;
    const scrollTop = container.scrollTop || 0;
    const viewportH = container.clientHeight || 0;
    webglPacketRenderer.render(xScale, scrollTop, viewportH);
}

function _attachCirclesScrollListener() {
    if (_circlesViewport.scrollAttached) return;
    const chartContainer = document.getElementById('chart-container');
    if (!chartContainer) return;
    chartContainer.addEventListener('scroll', () => {
        if (_circlesViewport.rafPending) return;
        _circlesViewport.rafPending = true;
        requestAnimationFrame(() => {
            _circlesViewport.rafPending = false;
            if (state.ui.renderMode !== 'packets') return;
            // Fetch a fresh small subset for the new scroll position directly,
            // bypassing the zoom-handler chain (which triggers transitions,
            // ground-truth redraws, etc. and caused per-scroll memory blow-up).
            try { _scrollRefetchPacketData(); } catch (e) { console.warn('[scroll] refetch failed', e); }
        });
    }, { passive: true });
    _circlesViewport.scrollAttached = true;
}

/**
 * Scroll-driven packet refresh. Picks up the current resolution's columnar
 * data, applies time + selected-IP-pair + scroll-visible-IP filters using
 * typed-array column ops (no row materialization), and only then materializes
 * the small surviving subset for the renderer. Bypasses the zoom-handler
 * chain so each scroll event does just this fixed bounded work.
 */
function _scrollRefetchPacketData() {
    const layer = _circlesViewport.layer;
    if (!layer || !xScale) return;
    const __dbgT0 = performance.now();
    let __dbgStage = 'init';

    // Pick a columnar source. Prefer the active resolution's preload; fall
    // back to state.data.full.
    let cols = null;
    if (typeof currentResolutionLevel === 'string' && fetchResManager
        && fetchResManager.singleFileData
        && fetchResManager.singleFileData.has(currentResolutionLevel)) {
        const pre = fetchResManager.singleFileData.get(currentResolutionLevel);
        if (isBinColumns(pre)) cols = pre;
    }
    if (!cols && isBinColumns(state.data.full)) cols = state.data.full;
    if (!cols) {
        console.log('[scrollRefetch] no columnar source — fallback');
        if (_circlesViewport.binned) _renderCirclesViewport(null);
        return;
    }
    __dbgStage = 'time';
    const [d0, d1] = xScale.domain();
    const totalIn = cols.length;
    let subset = cols.filterByTimeRange(Math.floor(d0), Math.floor(d1));
    const afterTime = subset.length;
    if (subset.length === 0) {
        console.log(`[scrollRefetch] EMPTY after time | in=${totalIn}`);
        _circlesViewport.binned = []; _renderCirclesViewport(null); return;
    }
    __dbgStage = 'pair';
    // Read selection from the actual checkbox state — fetchResManager's
    // selectedIPSet bookkeeping isn't synced on the initial render path.
    let selectedIPs = (fetchResManager && fetchResManager.selectedIPSet && fetchResManager.selectedIPSet.size >= 2)
        ? fetchResManager.selectedIPSet
        : null;
    if (!selectedIPs) {
        const checked = document.querySelectorAll('#ipCheckboxes input[type="checkbox"]:checked');
        if (checked.length >= 2) {
            selectedIPs = new Set();
            for (const cb of checked) selectedIPs.add(cb.value);
        }
    }
    if (!selectedIPs || selectedIPs.size < 2) {
        console.log(`[scrollRefetch] EMPTY no/few selected IPs | sel=${selectedIPs ? selectedIPs.size : 'null'}`);
        _circlesViewport.binned = []; _renderCirclesViewport(null); return;
    }
    subset = subset.filterByIpPair(selectedIPs);
    const afterPair = subset.length;
    if (subset.length === 0) {
        console.log(`[scrollRefetch] EMPTY after IP-pair | time=${afterTime} sel=${selectedIPs.size}`);
        _circlesViewport.binned = []; _renderCirclesViewport(null); return;
    }
    __dbgStage = 'visible';
    const visibleIPs = _circlesViewportIPSet();
    if (visibleIPs && visibleIPs.size > 0) {
        subset = subset.filterByIpRowSet(visibleIPs);
    } else {
        console.log(`[scrollRefetch] DEFERRED — layout not ready | visibleIPs=${visibleIPs ? visibleIPs.size : 'null'}`);
        return;
    }
    const afterVis = subset.length;
    if (subset.length === 0) {
        console.log(`[scrollRefetch] EMPTY after visible-IP | pair=${afterPair} vis=${visibleIPs.size}`);
        _circlesViewport.binned = []; _renderCirclesViewport(null); return;
    }
    _circlesViewport.binned = subset.toRowArray();
    _renderCirclesViewport(null);
    console.log(`[scrollRefetch] OK in=${totalIn} time=${afterTime} pair=${afterPair} vis=${afterVis} (${(performance.now()-__dbgT0).toFixed(1)} ms) stage=${__dbgStage}`);
}
...
function renderCirclesWithOptions(layer, binned, rScale, transitionOpts) {
    _circlesViewport.layer = layer;
    _circlesViewport.binned = binned;
    _circlesViewport.rScale = rScale;
    _renderCirclesViewport(transitionOpts);
    _attachCirclesScrollListener();
}

/**
 * Attach a single mousemove handler (only once per page lifetime) to the
 * chart-container for hit-testing the WebGL packet renderer. The handler
 * reads `webglPacketRenderer`, `xScale`, `webglPacketMargin` at call time
 * so it stays correct across visualizeTimeArcs re-runs.
 */
let _webglHoverAttached = false;
function _attachWebGLHoverHandlers() {
    if (_webglHoverAttached) return;
    const container = document.getElementById('chart-container');
    if (!container) return;
    ...
    const onMove = (event) => {
        ...
        if (onMove._raf) return;
        onMove._raf = requestAnimationFrame(() => {
            onMove._raf = null;
            const e = pendingMove;
            if (!e || !webglPacketRenderer || state.ui.renderMode !== 'packets') return;
            const margin = webglPacketMargin;
            if (!margin) return;
            const svgNode = svg && svg.node()?.ownerSVGElement;
            if (!svgNode) return;
            const rect = svgNode.getBoundingClientRect();
            const px = e.clientX - rect.left - margin.left;
            const py = e.clientY - rect.top - margin.top;
            const hit = webglPacketRenderer.hitTest(px, py, xScale);
            if (hit) {
                if (webglHoverIndex !== hit.index) {
                    webglHoverIndex = hit.index;
                    webglPacketRenderer.setHover(hit.index);
                    _drawWebGLPackets();
                }
                tooltip.style('display', 'block')
                    .style('left', `${e.pageX + 40}px`)
                    .style('top', `${e.pageY - 40}px`)
                    .html(createTooltipHTML(hit.datum));
                const d = hit.datum;
                const dstIps = new Set((d.ipPairs || [{ dst_ip: d.dst_ip }]).map(p => p.dst_ip));
                try { onCircleHighlight(d.src_ip, dstIps); } catch {}
            } else if (webglHoverIndex !== -1) {
                webglHoverIndex = -1;
                webglPacketRenderer.setHover(null);
                _drawWebGLPackets();
                tooltip.style('display', 'none');
                try { onCircleClearHighlight(); } catch {}
            }
        });
    };
    const onLeave = () => {
        if (webglHoverIndex !== -1) {
            webglHoverIndex = -1;
            if (webglPacketRenderer) {
                webglPacketRenderer.setHover(null);
                _drawWebGLPackets();
            }
            tooltip.style('display', 'none');
...
function renderLozengesWithOptions(layer, flowData, transitionOpts) {
    ...
    // Canvas fast path: render lozenges + labels together
    if (canvasRenderer) {
...
async function switchViewMode(mode, { force = false } = {}) {
    ...
    if (mode === 'flows') {
        ...
        if (fullDomainLayer) {
            ...
            fullDomainLayer.selectAll('.sub-row-ip-label').remove();
        }
        if (dynamicLayer) {
            ...
            dynamicLayer.selectAll('.sub-row-ip-label').remove();
        }
        if (webglPacketRenderer) webglPacketRenderer.hide();
        ...
        if (webglPacketRenderer) webglPacketRenderer.show();
...
function applySearchHighlightClasses() {
    ...
    if (!state.search.active || !state.search.results) {
        ...
        if (webglPacketRenderer) webglPacketRenderer.setOpacityArray(null);
    ...
    // Backend-agnostic per-circle visitor. SVG path reads attrs from DOM;
    // WebGL path reads from the renderer's CPU-side arrays + xScale.
    const useWebGLBackend = !!webglPacketRenderer && webglPacketRenderer.items;
    const visit = (d, cx, cy, r, setOpacity) => {
        if (!d) return;
        ...
        if (isMatch) {
            ...
            setOpacity(filterActive ? 0.12 : 1);
        }
    };

    if (useWebGLBackend) {
        const items = webglPacketRenderer.items;
        const xs = webglPacketRenderer.xArr;
        const ys = webglPacketRenderer.yArr;
        const rs = webglPacketRenderer.rArr;
        const ops = new Float32Array(items.length);
        ops.fill(1);
        for (let i = 0; i < items.length; i++) {
            const d = items[i];
            const cx = xScale(xs[i]);
            visit(d, cx, ys[i], rs[i], (v) => { ops[i] = v; });
        }
        webglPacketRenderer.setOpacityArray(ops);
        _drawWebGLPackets();
    } else {
        mainGroup.selectAll('.direction-dot').each(function(d) {
            const el = d3.select(this);
            const cx = +el.attr('cx'), cy = +el.attr('cy'), r = +el.attr('r') || 3;
            visit(d, cx, cy, r, (v) => el.style('opacity', v === 1 ? null : v));
        });
    }
    ...
    reader.onload = async e => {
        try {
            ...
            if (packets && packets.length > 0) {
                ...
                const uniqueIPs = extractUniqueIPsFromPackets(state.data.full);
...
function visualizeTimeArcs(packets) {
    ...
    // 9b. Create the WebGL packet renderer (or reuse — the canvas lives in
    // #chart-container, parent of #chart, so it survives the html("") above).
    // Reusing the GL context across visualizeTimeArcs avoids exhausting the
    // browser's per-page WebGL context pool (~16).
    try {
        const chartContainer = document.getElementById('chart-container');
        if (chartContainer) {
            if (!webglPacketRenderer) {
                webglPacketRenderer = new WebGLPacketRenderer(chartContainer, margin);
            } else {
                webglPacketRenderer.setMargin(margin);
            }
            webglPacketRenderer.setTotalWidth(width + margin.left + margin.right);
            webglPacketMargin = margin;
            if (state.ui.renderMode !== 'packets') webglPacketRenderer.hide();
            else webglPacketRenderer.show();
            _attachWebGLHoverHandlers();
        }
    } catch (e) {
        console.warn('[WebGL] init failed, falling back to SVG circles:', e);
        webglPacketRenderer = null;
    }
...
function _initFlowOnlyChart(selectedIPs, timeExtent) {
    ...
    const FLOW_ROW_GAP = 0.1; // sub-pixel rows for flow-only mode
    const margin = { top: 80, right: 120, bottom: 50, left: 180 };
    width = d3.select("#chart-container").node().clientWidth - margin.left - margin.right;
    ...
    // IP positioning: one row per IP, compact 10px gap
    state.layout.ipPositions.clear();
    ...
    state.layout.ipOrder = selectedIPs;
    let y = TOP_PAD;
    for (const ip of selectedIPs) {
        state.layout.ipPositions.set(ip, y);
        state.layout.ipRowHeights.set(ip, FLOW_ROW_GAP);
        state.layout.ipPairCounts.set(ip, 1);
        state.layout.collapsedIPs.add(ip);
        y += FLOW_ROW_GAP;
    }
    height = y + FLOW_ROW_GAP;
    const minY = TOP_PAD;
    const maxY = y;
    ...
    // Initialize canvas renderer for lozenges + labels
    try {
        if (canvasRenderer) canvasRenderer.destroy();
        const chartContainer = document.getElementById('chart-container');
        canvasRenderer = new CanvasFlowRenderer(chartContainer, margin, width);
        canvasRenderer.setLayout(selectedIPs, state.layout.ipPositions, FLOW_ROW_GAP);
        console.log('[FlowOnly] Canvas flow renderer initialized');

        // Enable hover for IP labels
        canvasRenderer.enableHover(xScale, () => chartContainer.scrollTop);

        // Scroll listener: re-render visible rows on scroll
        chartContainer.addEventListener('scroll', () => {
            if (canvasRenderer && xScale) {
                canvasRenderer.render(xScale, chartContainer.scrollTop, chartContainer.clientHeight);
            }
        });
    } catch (e) {
        console.warn('[FlowOnly] Canvas renderer failed:', e.message);
        canvasRenderer = null;
    }
...
Check browser console (F12) for detailed error logs.`);
        ...
        if (preloadedUniqueIPs && preloadedUniqueIPs.length > 0) {
            ...
            uniqueIPs = extractUniqueIPsFromPackets(state.data.full);
...
function getVisibleTimeExtent() {
    ...
    if (state.data.full && state.data.full.length > 0) {
        // Column-aware fast path: read the timestamps Float64Array directly.
        if (isBinColumns(state.data.full)) {
            const ts = state.data.full.timestamps;
...
async function fetchGetMultiResData(xScale, zoomLevel) {
    ...
    // Column-aware shrink: when input is BinColumns we apply time + IP-pair
    // + scroll-visible-IP filters using the typed-array columns directly,
    // then materialize only the small surviving subset to a plain Array
    // (which the shared zoom handler's `.map(d => ({...d, ...}))` then
    // operates on cheaply). Without this, the shared .map allocates 8M
    // wrapper objects on the full dataset and OOMs the tab.
    const shrinkColumnar = (cols) => {
        // 1. time range
        cols = cols.filterByTimeRange(start, end);
        if (cols.length === 0) return [];
        // 2. selected-IP-pair (both endpoints in the user's IP selection)
        const selectedIPs = fetchResManager.selectedIPSet;
        if (!selectedIPs || selectedIPs.size < 2) return [];
        cols = cols.filterByIpPair(selectedIPs);
        if (cols.length === 0) return [];
        // 3. scroll-visible IP rows — bounds the per-render cost regardless
        //    of selection size. Without this, "all IPs selected" still
        //    yields multi-million-row subsets.
        const visibleIPs = _circlesViewportIPSet();
        if (visibleIPs && visibleIPs.size > 0) {
            cols = cols.filterByIpRowSet(visibleIPs);
            if (cols.length === 0) return [];
        }
        return cols.toRowArray();
    };
    ...
    if (resConfig?.isSingleFile) {
        ...
        if (preloadedData) {
            if (isBinColumns(preloadedData)) {
                return { data: shrinkColumnar(preloadedData), resolution, preBinned: resConfig.preBinned };
            }
        ...
        if (isBinColumns(state.data.full)) {
            return { data: shrinkColumnar(state.data.full), resolution, preBinned: true };
        }
 ...
 */
// Lazy-loaded hyparquet ESM module, shared across all chunked-Parquet loads.
let _hyparquetTfaPromise = null;
async function _getHyparquetTfa() {
    if (!_hyparquetTfaPromise) {
        ...
        _hyparquetTfaPromise = import('https://esm.sh/hyparquet@1.25.6');
    }
    return _hyparquetTfaPromise;
}

const _RESOLUTION_BIN_SIZE_US_TFA = {
    'hours':   3_600_000_000,
    'minutes': 60_000_000,
    '10s':     10_000_000,
    'seconds': 1_000_000,
 ...
 * for binned data (see parsePacketCSV in this file). Schema (v3.5):
 *   timestamp:int64, src_ip:str, dst_ip:str, count:int, total_bytes:int, flag_type:str
 * hyparquet returns int64 columns as BigInt; coerce via Number().
 */
async function _loadParquetBinChunk(url, resName) {
    const binSize = _RESOLUTION_BIN_SIZE_US_TFA[resName] || 1_000_000;
    const hp = await _getHyparquetTfa();
    const resp = await fetch(url);
    if (!resp.ok) throw new Error(`HTTP ${resp.status} for ${url}`);
    const buf = await resp.arrayBuffer();
    const rawRows = await hp.parquetReadObjects({ file: buf });

    // Decode parquet rows directly into columnar typed arrays + string
    // dictionaries. Avoids building 8M+ JS objects (which would peak ~1.5 GB
    // of heap for the minute tier alone). Per-row cost in this layout is
    // 8+4+4+4+4+1 = 25 bytes — about 60x smaller than the equivalent
    // plain-object array.
    const n = rawRows.length;
    const timestamps = new Float64Array(n);
    const srcIpIds   = new Uint32Array(n);
    const dstIpIds   = new Uint32Array(n);
    const counts     = new Uint32Array(n);
    const totalBytes = new Uint32Array(n);
    const flagIds    = new Uint8Array(n);
    ...
    for (let i = 0; i < n; i++) {
        const r = rawRows[i];
        timestamps[i] = Number(r.timestamp);
        srcIpIds[i]   = internIp(String(r.src_ip || ''));
        dstIpIds[i]   = internIp(String(r.dst_ip || ''));
        counts[i]     = Number(r.count) || 0;
        totalBytes[i] = Number(r.total_bytes) || 0;
        flagIds[i]    = internFlag(String(r.flag_type || 'OTHER'));
    }
    // Drop the parquet-decoded plain objects so they GC promptly. The columnar
    // arrays above are the only retained representation.
    rawRows.length = 0;

    return new BinColumns({
        length: n,
        timestamps, srcIpIds, dstIpIds, counts, totalBytes, flagIds,
        ipDict, flagDict,
        binSize,
        resolution: resName
    });
...
async function loadResolutionPackets(basePath, resName, opts = {}) {
    ...
    // Chunked layout (v3.4 CSV / v3.5 Parquet — distinguished by file extension)
            ...
            index.chunks.map(async (chunk) => {
                ...
                try {
                    if (typeof chunk.file === 'string' && chunk.file.endsWith('.parquet')) {
                        return await _loadParquetBinChunk(url, resName);
                    }
                    ...
                    console.warn(`[loadResolutionPackets] Skipping ${chunk.file}:`, err.message);
                    return [];
                }
            })
        );
        // If every chunk returned a BinColumns (parquet path), concat them in
        // typed-array form to keep memory low. Otherwise fall back to Array
        // flatten for the legacy CSV path. The columnar result is wrapped in
        // an array-like Proxy so shared code that does indexed access (e.g.
        // initialRender.js's `arr[i]` loop) still works.
        const allColumnar = chunkPackets.every(p => isBinColumns(p));
        if (allColumnar) {
            const merged = BinColumns.concat(chunkPackets);
            console.log(`[loadResolutionPackets] ${resName}: parsed ${merged.length} rows from ${index.chunks.length} parquet chunks (columnar)`);
            return asArrayLike(merged);
        }
    ...
    // Single-file layout (v3.3 legacy)
...
function extractUniqueIPsFromPackets(packets) {
    // Fast path for columnar packets — the IP dictionary is already the
    // unique set, no need to materialize and re-deduplicate 8M+ rows.
    if (isBinColumns(packets)) {
        return packets.ipDict.slice().filter(Boolean).sort();
    }
    const ips = new Set();
    for (const p of packets) {
